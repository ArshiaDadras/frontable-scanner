#!/usr/bin/env bash
# find_address.sh — Test different address configurations
...
while true; do
  read -p "Enter path to your v2ray addresses file: " ADDRESS_PATH
  if [[ -f "$ADDRESS_PATH" ]]; then
    V2RAY_ADDRESS="$ADDRESS_PATH"
    break
  else
    log error "File not found: $ADDRESS_PATH"
  fi
done
...
######################## Setup Results ####################################
RESULTS_DIR="$(pwd)/output/address"
mkdir -p "$RESULTS_DIR"
RESULTS_FILE="$RESULTS_DIR/results-$(date +%F-%H%M%S).txt"

echo "# Address Test Results" > "$RESULTS_FILE"
...
test_address_config() {
  ...
  log info "Testing: address=$address" >&2
  
  # Update config with new address settings
  python3 py/address.py "$V2RAY_CONFIG" "$address"
  ...
  if ! pgrep -f "$V2RAY_BINARY.*$V2RAY_CONFIG" >/dev/null 2>&1; then
    ...
    printf "%-15s %-15s %-12s %-12s %-12s %-12s %s\n" \
      "$address" "FAILED" "0KB/s" "0ms" "0.00" >> "$RESULTS_FILE"
    rm -f "$v2ray_log"
    echo "0"
    return 0
  fi
...
log info "Starting address configuration tests..."
...
for address in "${SHUFFLED_CONFIGS[@]}"; do
  ...
  score=$(test_address_config "$address") || true
  
  if [[ $score -gt $BEST_SCORE ]]; then
    BEST_SCORE=$score
    BEST_ADDRESS="address=$address"
...
echo "================================================================================"
echo ""
log info "Best Address:"
echo "  $BEST_ADDRESS"
echo "  Score: $(echo "scale=2; $BEST_SCORE / 100" | bc)"
echo ""
log info "Detailed results saved to: $RESULTS_FILE"
echo ""
log info "To apply the best address, update your v2ray config address settings:"
echo "  ${CYAN}$BEST_ADDRESS${NONE}"
echo ""
